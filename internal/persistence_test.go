...
func TestStorePersistence(t *testing.T) {
	store := NewStore[int, int](1000, false, nil, nil, nil, 0, 0, false)
	for i := 0; i < 20; i++ {
		_ = store.Set(i, i, 1, 0)
	}
	...
	new := NewStore[int, int](1000, false, nil, nil, nil, 0, 0, false)
...
func TestStorePersistenceResize(t *testing.T) {
	...
	new := NewStore[int, int](100, false, nil, nil, nil, 0, 0, false)
	f, err = os.Open("stest")
	require.Nil(t, err)
	err = new.Recover(0, f)
	require.Nil(t, err)
